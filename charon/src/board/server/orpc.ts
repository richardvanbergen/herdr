...
import { asc, eq, isNull, getTableColumns } from "drizzle-orm";
...
export const getBoard = os.handler(() => {
	const columnRows = db
		...
		.all();
	const placements = db
		.select(getTableColumns(jobPlacements))
		.from(jobPlacements)
		.innerJoin(jobs, eq(jobs.id, jobPlacements.jobId))
		.where(isNull(jobs.deletedAt))
		.orderBy(asc(jobPlacements.position))
		.all();
	return {
		columns: columnRows.map((column) => ({
			...column,
			jobIds: placements
				.filter((placement) => placement.columnId === column.id)
				.map((placement) => placement.jobId),
		})),
	};
});

export const addJob = os
	.input(
		z.object({
	...
	.handler(({ input }) =>
		db.transaction((tx) => {
			...
			const job = tx
				...
				.get();
			const placement = tx
				.select({ position: jobPlacements.position })
				.from(jobPlacements)
				.where(eq(jobPlacements.columnId, input.columnId))
				.orderBy(asc(jobPlacements.position))
				.all();
			tx.insert(jobPlacements)
				.values({
					jobId: job.id,
	...
	.handler(({ input }) =>
		db.transaction((tx) => {
			...
			const placement = tx
				.select()
				.from(jobPlacements)
				.where(eq(jobPlacements.jobId, input.jobId))
				.get();
			...
			const placements = tx
				...
				.orderBy(asc(jobPlacements.position))
				.all();
			const sourceIds = placements
				.filter(
					(item) =>
						item.columnId === placement.columnId && item.jobId !== input.jobId,
				)
				.map((item) => item.jobId);
			const destinationIds =
				placement.columnId === input.columnId
					? sourceIds
					: placements
							.filter((item) => item.columnId === input.columnId)
							.map((item) => item.jobId);
			destinationIds.splice(
				Math.min(input.index, destinationIds.length),
				0,
				input.jobId,
			);
			...
			for (const [position, jobId] of destinationIds.entries()) {
				tx.update(jobPlacements)
					.set({ columnId: input.columnId, position })
					.where(eq(jobPlacements.jobId, jobId))
					.run();
			}
			return { success: true };
